...
You can see that each sample is assigned a different combination of study ID and grid cell (based on its latitude and longitude) resulting in a unique identifier which determines our assemblage time-series (assemblageID). Let’s take a look and see how many studies/cells/assemblages time-series are there:

```{r gridding_ex_plot, cache=TRUE, echo=TRUE, message=FALSE, fig.width=7, fig.height=3,tidy=TRUE, include=TRUE} 
check1 <- grid_samples %>% 
...
ggplot(data = check1) +
  geom_bar(mapping = aes(x = as.character(STUDY_ID), y = n_aID, fill=res ,),stat = "identity")+
  scale_fill_discrete(type=c("#155f49"))+
  xlab("StudyID") + ylab("Number of assemblages in a study")+
  theme_bw()
```
You can see that several studies are not partitioned, that's because all their observations were already contained within a single cell.
...
## The following example is build on the demonstration show in
...
#To make it lighter, we only map the grid cell boundaries for 
#cells which we have observations.
...
```
Do you see why reflecting on your `res` may the important? - You can see that the highest the resolution you set the highest the number of cells in which samples get divided. However there is also the risk of the resulting assemblages time-series becoming to small to have any ecological meaning. A good rule of thumb is to never set `res` higher than the resolution you get when `resByData = TRUE`.


### Why rarefaction is a must!
Now we will deal with another common issue when working with time-series data, the fact that the number of locations sampled trough time may varied across time. A rather common phenomenon in BioTIME studies as well. Why this may be a problem you ask? - Well when looking at changes trough time it's important to be able to quantify change independently of the number of samples e.g. alpha diversity has truly increase and is not a sampling artifact resulting from expanding my original sampling of 3 plots to 5 plots in latest years. 
...
#let's apply it then:
grid_rare<- resampling(x = grid_samples, measure ="ABUNDANCE", resamps = 1L, conservative = FALSE)
```
...
A way to ensure this doesn’t become a problem, is to repeat this process multiple times, effectively creating multiple alternative datasets that can be used in your subsequent sensitivity analysis:
...
alpha_slopes_simp<-as.data.frame(alpha_slopes_simp %>% separate(., assemblageID, 
...
```{r trends7, cache=TRUE, echo=TRUE, message=FALSE, fig.width=7.2, fig.height=1.9,tidy=TRUE, include=TRUE,results='hold'} 
#Now you can see the full distribution of slopes. On the background, in grey, all assemblages 
# are shown, while in color we only show the subset of assemblages for which
...
#Let's go back to the dataframe with the yearly values and select our relevant data for plotting
alpha_slopes_set <- subset(alpha_slopes, assemblageID == "211_528514")
...
ggplot(data = alpha_slopes_set, aes(x = YEAR, y = diversity)) +
      ...
      stat_smooth(data = subset(alpha_slopes_set, alpha_slopes_set$significance == 1), #color only trends that are significant
...
You can see that results may vary depending on your dataset and the metric of change you use. Is important to remember that a biodiversity metric is just a tool used by ecologists to measure changes in biodiversity, and can show diverse abilities to detect changes under different scenarios. While they are complementary, they also provide different pieces of a complex puzzle and may strongly affect our interpretation of biodiversity change. Therefore before deciding which metric of set of metrics you deem of interest in your analytical approach, is crucial that you reflect which metrics respond to certain changes, as some may be more appropriate then others to answer your specific research question. 
