...
```{r trends8_hidden, cache=TRUE, echo=FALSE, message=FALSE, fig.width=7.2, fig.height=6,tidy=FALSE, include=FALSE, eval=TRUE, results='hold', figures='hold'} 
...
#Let's put it all together
if (!file.exists("trends8.rds")) {
  p <- ggplot(data = alpha_slopes_set2, aes(x = YEAR, y = diversity)) +
  ...
  saveRDS(p, file = "trends8.rds")
} else {
  p <- readRDS(file = "trends8.rds")
}
p

```

```{r trends8_shown, cache=FALSE, echo=FALSE, message=FALSE, tidy=FALSE, include=TRUE, eval=FALSE,results='hold', figures='hold'} 
# Let's go back to the data frame with the yearly values and select our relevant data for plotting
alpha_slopes_set2 <- subset(alpha_slopes, assemblageID == "18_335699")
...
citation("BioTIMEr")
```
Sagouis A, Moyes F, Martins IS, Blowes SA, Brambilla V, Chow CFY, Fontrodona-Eslava A, Antão L, Chase JM,
