...
getLinearRegressions <- function(x, divType) {
  ...
  suppressWarnings({
    ...
    if (divType == "alpha") {
      ...
      ###############################################
      b1 <- dplyr::select(dft, rarefyID, SSl, NS, simpS, invSSl, domMS,
                   PIES, expShS, shanS)
      colnames(b1) <- c("rarefyID", "S", "N", "Simpson", "InvSimpson",
                        "DomMc", "PIE", "expShannon", "Shannon")
      d1 <- as.data.frame(tidyr::pivot_longer(b1, -rarefyID, names_to = "metric",
                                       values_to = "slopes"))
      b2 <- dplyr::select(dft, rarefyID, SPval, NPval, simpPval, invSPval, domMPval,
                   PIEPval, expShPval, shanPval)
      colnames(b2) <- c("rarefyID", "S", "N", "Simpson", "InvSimpson",
                        "DomMc", "PIE", "expShannon", "Shannon")
      d2 <- as.data.frame(tidyr::pivot_longer(b2, -rarefyID, names_to = "metric",
                                       values_to = "p-values"))
      d3 <- merge(d1, d2, by = c("rarefyID", "metric"))
      b3 <- dplyr::select(dft, rarefyID, sp, np, sip, isp, dmp, pp, esp, shp)
      colnames(b3) <- c("rarefyID", "S", "N", "Simpson", "InvSimpson",
                        "DomMc", "PIE", "expShannon", "Shannon")
      d4 <- as.data.frame(tidyr::pivot_longer(b3, -rarefyID, names_to = "metric",
                                       values_to = "significance"))
      d5 <- merge(d3, d4, by = c("rarefyID", "metric"))
      b4 <- dplyr::select(dft, rarefyID, SSI, NI, simpI, invSI, domMI,
                   PIEI, expShI, shanI)
      colnames(b4) <- c("rarefyID", "S", "N", "Simpson", "InvSimpson",
                        "DomMc", "PIE", "expShannon", "Shannon")
      d8 <- as.data.frame(tidyr::pivot_longer(b4, -rarefyID, names_to = "metric",
                                       values_to = "intercept"))
      d7 <- merge(d5, d8, by = c("rarefyID", "metric"))
      d6 <- as.data.frame(tidyr::pivot_longer(x, -c(rarefyID, Year),
    ...
    if (divType == "beta") {
      ...
      #####################################
      b1 <- dplyr::select(dft, rarefyID, jdS, mhS, bcS)
      colnames(b1) <- c("rarefyID", "JaccardDiss", "MorisitaHornDiss",
                        "BrayCurtisDiss")
      d1 <- as.data.frame(tidyr::pivot_longer(b1, -rarefyID, names_to = "metric",
                                       values_to = "slopes"))
      b2 <- dplyr::select(dft, rarefyID, jdPval, mhPval, bcPval)
      colnames(b2) <- c("rarefyID", "JaccardDiss", "MorisitaHornDiss",
                        "BrayCurtisDiss")
      d2 <- as.data.frame(tidyr::pivot_longer(b2, -rarefyID, names_to = "metric",
                                       values_to = "p-values"))
      d3 <- merge(d1, d2, by = c("rarefyID", "metric"))
      b3 <- dplyr::select(dft, rarefyID, jdp, mhp, bcp)
      colnames(b3) <- c("rarefyID", "JaccardDiss", "MorisitaHornDiss",
                        "BrayCurtisDiss")
      d4 <- as.data.frame(tidyr::pivot_longer(b3, -rarefyID, names_to = "metric",
                                       values_to = "significance"))
      d5 <- merge(d3, d4, by = c("rarefyID", "metric"))
      b4 <- dplyr::select(dft, rarefyID, jdI, mhI, bcI)
